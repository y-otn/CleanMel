...
def pad_or_cut_sample(wav, length, rng):
    while length > len(wav):
        wav = np.concatenate([wav, wav])
    if length < len(wav):
        if (len(wav) - length) > length // 4:
            # Split wav into 10 chunks
            chunk_hop = (len(wav) - length) // 10
            # calculate 10 chunks energy
            chunks = [wav[i:i + length] for i in range(0, len(wav) - length + 1, chunk_hop)]
            chunk_energies = [np.sum(chunk**2) / np.prod(chunk.shape) for chunk in chunks]
            # pick up chunk with max energy
            max_energy_idx = np.argmax(chunk_energies)
            wav = chunks[max_energy_idx]
        else:
            start = rng.integers(low=0, high=len(wav) - length + 1)
            wav = wav[start:start + length]
    return wav
# np.sum(wav1**2) / np.prod(wav1.shape)
def pad_or_cut_sample_with_st(wav, length, st):
    while length > len(wav):
        wav = np.concatenate([wav, wav])
    if length < len(wav):
        start = st
