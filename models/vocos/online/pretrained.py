...
from models.vocos.offline.feature_extractors import FeatureExtractor, EncodecFeatures
from models.vocos.online.heads import FourierHead
from models.vocos.online.models import Backbone
...
class Vocos(nn.Module):
    ...
    def forward(self, features: torch.Tensor, mag_recurrsive: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        """
        ...
        audio_output = self.decode(features, mag_recurrsive=mag_recurrsive, **kwargs)
        return audio_output

    @torch.inference_mode()
    def decode(self, features_input: torch.Tensor, mag_recurrsive: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        """
...
if __name__ == "__main__":
    model = Vocos.from_pretrained(
        ...
        "/nvmework3/shaonian/MelSpatialNet/MelSpatialNet/models/vocos/pretrained/vocos_hop256_clip1e-5_online.ckpt").to('cuda')
    x = torch.randn(1, 80, 251)  
    x = x.to('cuda')
    
    from torchinfo import summary
    summary(model, input_data=(x, x.mean(dim=1)), device='cuda')
    print(model)
    # from torch.utils.flop_counter import FlopCounterMode # requires torch>=2.1.0
    # with FlopCounterMode(model, display=False) as fcm:
    #     y = model.decode(x, x.mean(dim=1))
