...
from models.vocos.offline.discriminators import MultiPeriodDiscriminator, MultiResolutionDiscriminator
from models.vocos.offline.feature_extractors import FeatureExtractor
from models.vocos.offline.heads import FourierHead
from models.vocos.offline.helpers import plot_spectrogram_to_numpy
from models.vocos.offline.loss import DiscriminatorLoss, GeneratorLoss, FeatureMatchingLoss, MelSpecReconstructionLoss
# from models.vocos.offline.models import Backbone
from models.vocos.offline.modules import safe_log
...
class VocosExp(pl.LightningModule):
    ...
    def forward(self, audio_input, mag_recurrsive=None, **kwargs):
        features = self.feature_extractor(audio_input, **kwargs)
        x = self.backbone(features, **kwargs)
        audio_output = self.head(x, mag_recurrsive=mag_recurrsive)
        return audio_output

    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        audio_input, audio_normed, mag_recurrsive = batch
        
        # train discriminator
        if optimizer_idx == 0 and self.train_discriminator:             
            with torch.no_grad():
                audio_hat = self(audio_normed, mag_recurrsive, **kwargs)
        ...
        if optimizer_idx == 1:
            ...
            # check if loss have NAN values
            if torch.isnan(loss):
                print("NAN Loss")
                # save the model
                torch.save(self.state_dict(), "model_nan.pth")
                # save batch
                torch.save(batch, "batch_nan.pth")
                exit()
    ...
    def on_validation_epoch_start(self):
        if self.evaluate_utmos:
            from models.vocos.metrics.UTMOS import UTMOSScore
    ...
    def validation_step(self, batch, batch_idx, **kwargs):
        ...
        if self.evaluate_periodicty:
            from models.vocos.metrics.periodicity import calculate_periodicity_metrics
