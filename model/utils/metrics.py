...
import numpy as np
from typing import *
from model.utils.dnsmos import deep_noise_suppression_mean_opinion_score
...
def cal_metrics_functional(
    ...
    for m in metric_list:
        ...
        if mname == 'dnsmos':
            ...
            for idx, mid in enumerate(['p808', 'sig', 'bak', 'ovr', "p808_raw", 'sig_raw', 'bak_raw', 'ovr_raw']):
...
def recover_scale(preds: Tensor, mixture: Tensor, scale_src_together: bool, norm_if_exceed_1: bool = True) -> Tensor:
    ...
    print("In recover_scale, preds:", preds.abs().max(), mixture.abs().max())
    print("In recover_scale, a:", a)
    preds = preds * a
