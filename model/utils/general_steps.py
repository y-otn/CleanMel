...
from model.utils import MyJsonEncoder
from model.utils.weightavg import weightavg
from model.utils.flops import write_FLOPs
...
def on_validation_epoch_end(self: pl.LightningModule, cpu_metric_input: List[Tuple[ndarray, ndarray, int]], N: int = 5) -> None:
    ...
    for k in cpu_metric_input[0][0]:
        ms = list(filter(None, [m[0][k.split("/")[-1].lower()] for m in cpu_metrics]))
        if len(ms) > 0:
...
def on_load_checkpoint(
    ...
    weightavg_opts: Union[int, str, List[str], Literal[None]] = None,
    compile: bool = True,
) -> None:
    """load checkpoint
    ...
    Args:
        ...
        weightavg_opts: opts for weightavg. Defaults to None.
        compile: whether the checkpoint is a compiled one. Defaults to True.
    """
    if weightavg_opts:
        ckpt = self.trainer.ckpt_path
        ckpts, state_dict = weightavg(opts=weightavg_opts, ckpt=ckpt)
        self.print(f'rank {self.trainer.local_rank}/{self.trainer.world_size}, weightavg {weightavg_opts}: {ckpts}')
...
def configure_optimizers(
    ...
    trained_parameters = [p for p in self.parameters() if p.requires_grad] if isinstance(self, pl.LightningModule) else self
    optimizer = getattr(torch.optim, optimizer)(trained_parameters, **optimizer_kwargs)
...
def test_setp_write_example(self, xr: Tensor, yr: Tensor, yr_hat: Tensor, sample_rate: int, paras: Dict[str, Any], result_dict: Dict[str, Any], wavname: str, exp_save_path: str):
    ...
    example_dir = os.path.join(exp_save_path, 'examples', paras[0]["saveto"])
    os.makedirs(example_dir, exist_ok=True)
    # save preds and targets for each speaker
    for i in range(yr.shape[1]):
        # write ys
        
        wav_path = os.path.join(example_dir, pattern.format(name=f"_spk{i+1}.flac"))
        write_wav(wav_path=wav_path, wav=yr[0, i])
        # write ys_hat
        wav_path = os.path.join(example_dir, pattern.format(name=f"_spk{i+1}_p.flac"))
        write_wav(wav_path=wav_path, wav=yr_hat[0, i])  #, norm_to=ys[0, i].abs().max())
    # write mix
    wav_path = os.path.join(example_dir, pattern.format(name=f"_mix.flac"))
